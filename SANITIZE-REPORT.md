...
| 来源行 | `来源：BV1j3YL6oEvs「四层工作边界」——…` | 泛化为行为说明（致谢移至 CREDITS.md） | 本仓约定：思想引用的出处在 CREDITS 统一登记 |
| 触发条件措辞 | `模式 B 活文档项目里…` | `活文档项目里…` | 出版环境无「模式 B」上下文，去掉冗余限定 |
| 短任务阈值 | `简单问答、短任务（<15 分钟当量）不触发` | `简单问答、短任务不触发` | 去本机经验量化表述，语义不变 |
| project-bootstrap §3.2 | 指向「handoff 技能」 | 指向「同仓库 `skills/handoff/`」 | 发布版改仓库相对引用 |

正文其余逐字一致（已用 difflib 比对确认）。新内容复扫：八类零新增命中。
